...
class GeocoderClient:
    def __init__(self, api_key: str, base_url: str = 'https://geocode-maps.yandex.ru') -> None:
    ...
    async def get_address(self, location: dto.Location) -> tp.Optional[str]:
        ...
        async with aiohttp.ClientSession(self.base_url) as session:
            async with session.get('/1.x/', params=params) as response:
                ...
                if not member:
                    return None
                return member[0]['GeoObject']['metaDataProperty']['GeocoderMetaData']['text']
